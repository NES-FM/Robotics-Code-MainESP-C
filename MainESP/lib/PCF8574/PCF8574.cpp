...
void PCF8574::attachInterrupt(){
	...
	if (_usingInterrupt){
		...
		// logln("Using interrupt pin (not all pin is interrupted)");
...
bool PCF8574::begin(){
	...
	if (writeMode>0 || readMode>0)
	{
		...
		// logln("Start end trasmission if stop here check pullup resistor.");
...
void PCF8574::pinMode(uint8_t pin, uint8_t mode, uint8_t output_start){
	...
	if (mode == OUTPUT){
		...
		// logln("Mode non supported by PCF8574")
	}
};
